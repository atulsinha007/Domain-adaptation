...
def closeness_cost(ind):
	...
	for source_instance, source_lab in zip(source_rest_arr, source_rest_label):
		...
		min_dist = np.inf
		source_instance = np.reshape(source_instance, (source_instance.shape[0], 1))
		

		transformed_source = np.dot(W, source_instance)
		transformed_source = np.ravel(transformed_source)
		...
		for target_instance, target_lab in zip(target_rest_arr, target_rest_label):
			...
			if source_lab == target_lab:
				min_dist = min(min_dist, dist(transformed_source, target_instance))
			#print(min_dist)
		sumi += min_dist
