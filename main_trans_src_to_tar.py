...
def test_it_with_bp(play=1, NGEN=10, MU=4 * 5, play_with_whole_pareto=0, post_st = ''):
	...
	stringh = "_with_bp" + str(play) + "_src_to_tar" + str(NGEN)+post_st
	fronts = tools.sortNondominated(pop, len(pop))
	...
	if play_with_whole_pareto or len(fronts[0]) < 30:
		...
		pareto_front = random.sample(fronts[0], 30)
	with open("./log_folder/log_pareto_mat_"+post_st+".pickle", "wb") as fp:
...
if __name__ == "__main__":
	logf = open("./log_folder/log_error_main_src_to_tar.txt", "a")
	show_error = False
	if not show_error:
		try:
			post_st = sys.argv[1]
